    ...
    <table class="table table-striped table-bordered table-hover" id="sample-table">
		<thead>
		 <tr>
				...
				<th width="350px">path路径</th>
				<th width="180px">分类名称</th>               
				<th width="250px">操作</th>
			</tr>
		</thead>
	<tbody>
	@foreach($goodstype as $a)
      <tr>
       <td>{{$a->id}}</td>
       <td>{{$a->pid}}</td>
       <td>{{$a->path}}</td>
       <td>{{$a->type}}</td> 
      <td class="td-manage">
        <a title="编辑" id="sort_update" href="/admin/sortads/update?id={{$a->id}}"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>      
        <a title="删除" href="javascript:;"  onclick="del({{$a->id}},this)" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
       </td>
      </tr>
      @endforeach
    </tbody>
...
<div class="sort_style_add margin" id="sort_style_add" style="display:none">
  	<div class="">
  		<form action="/admin/sortads/add" method="post">
	     	...
	     	<ul>
		      		...
		      		<div class="col-sm-9">
		      			<select name="pid" class="input w50">
		            		<option value="">请选择分类</option>
		              		@foreach ($type1 as $v)
				              	<option value="{{$v->id}}">@for ($i = 1;$i < $v->num;$i++)--@endfor{{$v->type}}</option>
				            @endforeach
		          		</select>
		      		</div>
		      	</li>
		      	<li>
			      	...
			      	<div class="col-sm-9">
			      		<input name="type" type="text" id="form-field-1" placeholder="" class="col-xs-10 col-sm-5">
			      	</div>
			    </li>
			    <li>
			      	<div class="col-sm-9">
...
function del(id,obj){
	if(confirm("您确定要删除吗?")){			
		$.ajax({
      url:"/admin/sortads/del",//请求的url地址
      type:"GET",//传值方式
      data:{"id":id},//传输数据
      success:function(data){
        if (data == "删除失败") {
         ...
         // window.location.href = '/admin/goods_type';
        }else if(data == "此类别是根类别，不可删除"){
          alert(data);
        }else if(data == "此类别含有子类，请先删除子类"){
          alert(data);
        }
      },
    });
